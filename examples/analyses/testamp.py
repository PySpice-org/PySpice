...
def set_model_qbc847b(circuit):
	model = 'qbc847b'
	circuit.model(model, 'npn', IS = 1.822E-14, NF = 0.9932, ISE = 2.894E-16, NE = 1.4, BF = 324.4,
...
def do_noise_analysis():
	...
	com = 0
	simulator = circuit.simulator(temperature=25, nominal_temperature=25)
	analysis = simulator.noise(output_node=n.n5, ref_node=com, src='Vin', variation='dec', pts=10, fstart=100, fstop=1e5, pts_per_summary=1)
...
def do_distortion_analysis(f2overf1):
	...
	com = 0
	simulator = circuit.simulator(temperature=25, nominal_temperature=25)
	analysis = simulator.distortion(variation='dec', pts=10, fstart=100, fstop=1e4, f2overf1=f2overf1)
...
if __name__ == '__main__':
	...
	if args.dump_circuit: dump_circuit()
	if args.do_ac: do_ac_analysis()
	if args.do_dc: do_dc_analysis()
	if args.do_pz: do_pz_analysis()
	if args.do_noise: do_noise_analysis()
	if args.do_distoh: do_distortion_analysis(None)
	if args.do_distos: do_distortion_analysis(0.9)
	sys.exit(0)
